...
const ChatPage = () => {
  const {
    role, // Lấy quyền tài khoản từ hook
    chatRooms,
    messages,
    activeRoomId,
    activeRoom,
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] bg-white rounded-xl shadow-xs border border-gray-100 overflow-hidden mt-1">
      ...
      {/* --- SIDEBAR: ẨN HOÀN TOÀN NẾU LÀ KHÁCH HÀNG ĐĂNG NHẬP --- */}
      {role !== 'customer' && (
      ...
      {/* --- KHU VỰC MAIN CHAT (Khách hàng sẽ chiếm trọn 100% chiều rộng màn hình) --- */}
      <div className="flex-1 flex flex-col bg-white min-w-0">
        ...
        {activeRoom ? (
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <span className="text-[11px] font-medium text-gray-500">
                    {activeRoom.isOnline ? 'Đang trực tuyến' : 'Ngoại tuyến'} {role !== 'customer' && `• ID: ${activeRoom.id?.slice(0,8)}...`}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 flex items-center justify-center text-gray-400 bg-[#F8FAFC]">
             Đang khởi tạo kết nối cuộc hội thoại...
          </div>
        )}
        ...
        {activeRoom && (
          <div 
            ...
            className="flex-1 p-6 overflow-y-auto bg-[#F8FAFC] flex flex-col gap-4 scrollbar-thin"
          >
            <div className="text-center mb-2">
              <span className="text-[10px] font-medium text-gray-400 bg-white px-3 py-1 rounded-full shadow-xs border border-gray-100">
                Hỗ trợ trực tuyến Labs Flow
              </span>
            </div>

            {messages.map((msg) => {
              // Quy ước phân định vị trí bong bóng chat động theo Vai trò (Dynamic Role Alignment)
              // Staff: sendertype = 1 là Mine (Bên phải). Customer: sendertype = 2 là Mine (Bên phải).
              const isMine = role === 'customer' ? msg.sendertype === 2 : msg.sendertype === 1;
              const msgTime = msg.createdate 
                ? new Date(msg.createdate).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }) 
                : msg.time;

              return (
                <div key={msg.id || msg.createdate} className={`flex flex-col max-w-[70%] ${isMine ? 'self-end items-end' : 'self-start items-start'}`}>
                  <div 
                    className={`px-4 py-2.5 rounded-[18px] text-[13px] shadow-sm whitespace-pre-wrap break-words ${
                      isMine 
                  ...
                  <span className="text-[10px] text-gray-400 mt-1 px-1">{msgTime}</span>
                </div>
              );
            })}
            ...
            {typingStatus && (
              <div className="text-[11px] text-gray-400 italic animate-pulse self-start px-1">
                {typingStatus}
              </div>
            )}
            ...
            <form onSubmit={handleSendMessage} className="flex items-end gap-2">
              ...
              <div className="flex-1 bg-gray-50 border border-gray-200 rounded-2xl px-4 py-2 focus-within:border-[#0052FF] focus-within:ring-1 focus-within:ring-[#0052FF] transition-all">
                <input
                  ...
                  placeholder={role === 'customer' ? "Nhập nội dung thắc mắc cần Labs hỗ trợ..." : "Nhập tin nhắn hỗ trợ khách hàng..."}
